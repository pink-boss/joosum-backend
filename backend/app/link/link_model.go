...
func (LinkModel) GetAllLinkByUserIdAndLinkBookId(userId string, linkBookId string, sort string, order string) ([]*Link, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	...
	cursor, err := db.LinkCollection.Find(ctx, bson.M{"user_id": userId, "link_book_id": linkBookId}, opts)
	if err != nil {
		return nil, err
	}
